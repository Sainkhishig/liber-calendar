import { CalendarMath } from "../lib/main"
...
export default {
	...
	daysOfWeek(weekStart: Date): Array<Date> {
		console.log("баганын тоо харуулж байна")		
		console.log("daysOfWeek")
		var countDay = new Date(weekStart.getFullYear(), weekStart.getMonth(), 0).getDate()
		console.log(weekStart.getFullYear())
		console.log(weekStart.getMonth())
		return [...Array(countDay)].map((_, i) => this.addDays(weekStart, i))
	},

	daysCountOfMonth(weekStart: Date): number {
		console.log("баганын тоо харуулж байна")		
		console.log("daysOfWeek")
		var countDay = new Date(weekStart.getFullYear(), weekStart.getMonth(), 0).getDate()
		console.log(weekStart.getFullYear())
	...
	daysOfMonth(weekStart: Date): Array<Date> {
		console.log("daysOfMonth")

		return [...Array(CalendarMath.daysCountOfMonth(weekStart))].map((_, i) => this.addDays(weekStart, i))
	},

	daysCountInMonth(month: number, year: number) {
	...
	beginningOfWeek(d: Date, startDow: number): Date {
		console.log("beginningOfWeek check")
		console.log(d)
		console.log(d.getDay())
		return this.addDays(d, (startDow - d.getDay() - 7) % -7)
	},

	//7honogoor moruudiig uusgene. 1 Orond n oroonuudiig uusgej
	endOfWeek(d: Date, startDow: number): Date {
		console.log("endOfWeek check")
		console.log(startDow)
		console.log(d)
		return this.addDays(this.beginningOfWeek(d, startDow), 14)
	},
	...
	instanceOfMonth: (d: Date): number => Math.ceil(d.getDate() / CalendarMath.daysCountOfMonth(d)),
	...
	//7honogoor moruudiig uusgene. combo soligdohod ajilana
	incrementPeriod(d: Date, uom: string, count: number) {
		console.log("incrementPeriod check "+count)
	...
	isoYearMonthDay(d: Date): string {
		...
		console.log(d)
		console.log(this.paddedDay(d))
		console.log(this.isoYearMonth(d))
	...
	isLastInstanceOfMonth(d: Date): boolean {
		return d.getMonth() !== this.addDays(d, CalendarMath.daysCountOfMonth(d)).getMonth()
	},
	...
	// 7 хоногийн гаригуудыг илгээнэ. 日,月,火,水,木,金,土
	getFormattedWeekdayNames(locale: string, format: DateTimeFormatOption, startingDayOfWeek: number): Array<string> {
		...
		// console.log("weekdayNames starting day of week"+[...Array(7)].map((_, i) => formatter.format(new Date(2017, 0, (i + 1 + startingDayOfWeek) % 7))))
		console.log("weekdaysArray")
		console.log([...Array(30)].map((_, i) => formatter.format(new Date(2017, 0, (i + 1 + startingDayOfWeek) % 7))))
		return [...Array(30)].map((_, i) => formatter.format(new Date(2017, 0, (i + 1 + startingDayOfWeek) % 7)))
	},
